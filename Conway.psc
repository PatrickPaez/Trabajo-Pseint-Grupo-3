SubProceso tabla1 <- tabla0 ( puntos) 
	Definir a, b como real; b<-0;
	Para a<-0 hasta 1 Con Paso 1 Hacer
		Escribir "   ",puntos[a,b],"  ||  ",puntos[a,b+1],"  ||  ",puntos[a,b+2],"  ||  ", puntos[a,b+3],"  ||  ",puntos[a,b+4];
		Escribir " =====++=====++======++=====++=====";
	FinPara
...
SubProceso tabla2 <- tabla ( celdas, vida ) 
	Definir a, b, vivir, tabla1 como real;
	Definir seguir Como Logico; seguir<-Verdadero; vivo<-99;
	Mientras seguir=Verdadero Hacer
		Mientras vivir<>0 Hacer
			Escribir "Ingrese el numero por favor"; Leer vivo; //Ingresar los valores para convertir a "vivas" las celdas.
			Si vivir<0 Entonces
				Escribir "Es un numero positivo";Esperar 3 Segundos; //Verificar que sea positivo (válido).
			SiNo
				Para a<-0 hasta 4 Con Paso 1 Hacer
					Para b<-0 hasta 4 Con Paso 1 Hacer
						si vivir=vida(a,b) Entonces
							vida(a,b)<-0;
						FinSi
					FinPara
				FinPara
